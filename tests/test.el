...
(defun test/report (text status fn)
  (let ((format-status (lambda ()
                         (let ((status-string (cond ((eq status 'success) "PASS")
                                                    ((eq status 'skip) "SKIP")
                                                    ((eq status 'fail) "FAIL"))))
                           (princ (format "[%s] %s" status-string text) #'external-debugging-output))))
    ...
    (if (not noninteractive)
        (with-current-buffer (get-buffer-create report-buffer)
          (let* ((start (point))
                 ...
                 (goto-diff (lambda () (interactive) (funcall fn))))
...
(defun test/run ()
  (interactive)
  (setq report-buffer "*REPORT EXPORT*")
  (let* ((base-dir (vc-git-root default-directory))
         (test-dir (concat base-dir "tests/")))
    (when (not base-dir)
...
(defun test/run-compile ()
  (interactive)
  (setq report-buffer "*REPORT COMPILE*")
  (let* ((base-dir (vc-git-root default-directory))
         (test-dir (concat base-dir "tests/")))
    (when (not base-dir)
      (error "Not in a git repository"))
    ...
    (with-current-buffer (get-buffer-create report-buffer)
      ...
      (erase-buffer))
    (dolist (org-file (directory-files-recursively test-dir "\\.org$"))
      (let* ((typst-file (format "%s.typ" (file-name-sans-extension org-file)))
             (typst-report-buffer (format "*Output %styp*" (file-name-sans-extension org-file))))
        (when (get-buffer typst-report-buffer)
          (kill-buffer typst-report-buffer))
        (if (not (file-exists-p typst-file))
            (test/report org-file 'skip nil)
          (let* ((exit-code (shell-command (format "typst c %s" typst-file) typst-report-buffer)))
              (test/report typst-file
                           (if (zerop exit-code)
                               'success
                             'fail)
                           (lambda () (find-file typst-file)))))))
    (switch-to-buffer report-buffer)
    (delete-other-windows)
    (read-only-mode 1)
    (when noninteractive
